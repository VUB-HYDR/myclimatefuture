  ...
  <div class:slider-container="{true}">
    ...
    >
      <div
        ...
        aria-valuenow="{value}"
        id="{id}"
        use:tooltip={{ content: `Born in ${value}`, trigger: 'manual', offset: [0, 10], showOnCreate: true, hideOnClick: false, placement: 'top', delay: 500 }}
      ></div>
      <div bind:this="{trackRef}" class:slider__track="{true}"></div>
      <div
        class:slider__filled-track="{true}"
        style="transform: translate(0, -50%) scaleX({left / 100})"
      ></div>
      <input
        type="hidden"
        class:slider__input="{true}"
        name="{name}"
        value="{value}"
        required="{required}"
        min="{min}"
    ...
    <input
      ...
      class:slider-text-input="{true}"
      class:text-input--light="{light}"
